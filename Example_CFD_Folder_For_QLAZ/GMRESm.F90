...
!----------------------------------------------------------------------
 ...
 1 continue
   ...
   do j = 1, m
      ...
      call psolve(n, z)
      call matvec(n,z, w)
!WRITE(*,*) 'HELLO IN GMRESM,RANK=',RANK,'J=',J
      ...
      call dgelsy(j+1,j,1,h_,m+1,p,m+1,piv,m,rankm,work,4*m+1,i)
      ...
      !tol=1e-4
      if(info==1.AND.RANK.EQ.0) print*, 'gmresm: it=', its,' res=', real(res) &
                  , 'tol=',tol
                                                              
      done = (res<=tol .or. its==imx .or. res>res_)
      CALL MPI_BCAST(done,1,MPI_LOGICAL,0, &
           MPI_COMM_WORLD,ierror)
      if(done .or. j==m) then
         IF(RANK.EQ.0) THEN
         if(del>0d0)  call hookstep(j,h,m,beta,del, y)
         END IF
         CALL MPI_BCAST(del,1,MPI_DOUBLE_PRECISION,0, &
               MPI_COMM_WORLD,ierror)
