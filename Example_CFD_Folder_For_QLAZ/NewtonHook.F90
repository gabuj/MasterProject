...
!----------------------------------------------------------------------
 ...
 subroutine newtonhook(f, df, dfp, sub, dp, m, n, gtol, &
   ...
   if(ginfo/=0) then 
      if(info==1.AND.RANK.EQ.0) print*, 'newton: unable to allocate memory'
      info = 4
      return
   end if
...
! dotprod_ms
   ...
   CALL MPI_BCAST(new_tol,1,MPI_DOUBLE_PRECISION,0, &
          MPI_COMM_WORLD,ierror)
   if(del<0d0)  new_del = new_tol / 10d0
   if(del<0d0)  mxdl_   = 1d99
   if(info==1.AND.RANK.EQ.0)  print*,'newton: nits=',new_nits,' res=',real(new_tol)
...
! saveorbit
   ...
   if(new_tol<tol.AND.RANK.EQ.0) then
      if(info==1.AND.RANK.EQ.0) print*, 'newton: input already converged'
      info = 0
      deallocate(v)
      return
   end if
   ...
   do while(.true.)
      ...
      if(new_del<mndl) then
         if(info==1.AND.RANK.EQ.0) print*, 'newton: trust region too small'
         info = 3
         deallocate(v)
         return      
      end if
      ...
      CALL MPI_BCAST(gdel,1,MPI_DOUBLE_PRECISION,0, &
            MPI_COMM_WORLD,ierror)
...
! getrhs
      ...
      CALL MPI_BCAST(new_tol,1,MPI_DOUBLE_PRECISION,0, &
            MPI_COMM_WORLD,ierror)
      CALL MPI_BCAST(snrm,1,MPI_DOUBLE_PRECISION,0, &
            MPI_COMM_WORLD,ierror)
      ...
      if(new_tol>tol__) then
         if(info==1.AND.RANK.EQ.0) print*, 'newton: accepting previous step'
         new_x   = x__
         new_fx  = fx__
         new_tol = tol__
         new_del = del__
      else if(ared<0d0) then
         if(info==1.AND.RANK.EQ.0) print*, 'newton: norm increased, try smaller step'
         new_del = snrm * 0.5d0
         ginfo   = 2
      else if(ared/pred<0.75d0) then
         if(info==1.AND.RANK.EQ.0)  print*, 'newton: step ok, trying smaller step'
         x__     = new_x
         fx__    = new_fx
         tol__   = new_tol
         ...
         if(info==1.AND.RANK.EQ.0) print*, 'newton: step good, took full newton step'
         new_del = min(mxdl_,snrm*2d0)
      else if(new_del<mxdl_*0.9d0) then
         if(info==1.AND.RANK.EQ.0) print*, 'newton: step good, trying larger step'
         x__     = new_x
         fx__    = new_fx
         tol__   = new_tol
         del__   = new_del
      ...
      CALL MPI_BCAST(ginfo,1,MPI_INTEGER,0, &
            MPI_COMM_WORLD,ierror)     
      ...
      CALL MPI_BCAST(tol,1,MPI_DOUBLE_PRECISION,0, &
            MPI_COMM_WORLD,ierror)
      if(new_tol<tol) then
         if(info==1.AND.RANK.EQ.0) print*, 'newton: converged'
         info = 0
         deallocate(v)
         return
      else if(new_nits==nits) then
         if(info==1.AND.RANK.EQ.0) print*, 'newton: reached max its'
         info = 2
         deallocate(v)
         return
      end if
